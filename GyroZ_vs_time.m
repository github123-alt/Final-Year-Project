...
y_min    = min(gyroZ(vis_mask)) - 0.05;
y_max    = max(gyroZ(vis_mask)) + 0.05;

%% Compute min/max of visible window
gZ_visible = gyroZ(vis_mask);
t_visible  = time_plot(vis_mask);

[gZ_max, idx_max] = max(gZ_visible);
[gZ_min, idx_min] = min(gZ_visible);

t_at_max = t_visible(idx_max);
t_at_min = t_visible(idx_min);

fprintf('\n--- Gyro Z Stats (visible window: 0 to 70s) ---\n');
fprintf('Maximum GyroZ : %.6f rad/s  at t = %.2f secs\n', gZ_max, t_at_max);
fprintf('Minimum GyroZ : %.6f rad/s  at t = %.2f secs\n', gZ_min, t_at_min);
fprintf('------------------------------------------------\n');

% Align Land xline with minimum gyroZ time
land_t = t_at_min;

%% Plot
figure('Name', 'Gyro Z vs Time', 'NumberTitle', 'off', ...
    'Position', [100, 100, 1100, 580]);

% Raw data
plot(time_plot, gyroZ, 'b-', 'LineWidth', 1.0);
hold on;

% Smoothed overlay
window    = 20;
gZ_smooth = movmean(gyroZ, window);
plot(time_plot, gZ_smooth, 'r-', 'LineWidth', 2);
...
% Mark MAX point
plot(t_at_max, gZ_max, 'v', ...
    'MarkerSize', 12, ...
    'MarkerFaceColor', 'k', ...
    'MarkerEdgeColor', 'k');
text(t_at_max + 0.5, gZ_max - 0.01, ...
    sprintf('  Max: %.6f rad/s @ %.2f s', gZ_max, t_at_max), ...
    'FontSize', 11, ...
    'FontWeight', 'bold', ...
    'Color', 'k', ...
...
% Mark MIN point
plot(t_at_min, gZ_min, '^', ...
    'MarkerSize', 12, ...
    'MarkerFaceColor', 'k', ...
    'MarkerEdgeColor', 'k');
text(t_at_min - 0.5, gZ_min + 0.03, ...
    sprintf('Min: %.6f rad/s @ %.2f s', gZ_min, t_at_min), ...
    'FontSize', 11, ...
    'FontWeight', 'bold', ...
    'Color', 'k', ...
    'BackgroundColor', [1.00 1.00 1.00], ...
...
ylabel('Gyro Z (rad/s)', 'FontSize', 13);
title('Gyro Z vs Time', 'FontSize', 15, 'FontWeight', 'bold');
legend('Raw GyroZ data', 'Smoothed (20-pt moving avg)', 'Location', 'northeast');
grid on;
